...
async def json_to_vtt(json_value1, name1):
    vtt_buffer = io.StringIO()    
    vtt_buffer.write("WEBVTT\n\n")
    file1 = json_value1.split("\n")
    for i in file1:
        vtt_buffer.write(i + "\n")
    with open(f"files/{name1}.vtt", "w", encoding="utf-8") as final_file:
        final_file.write(vtt_buffer.getvalue())
    vtt_file = webvtt.read(f"files/{name1}.vtt")
    vtt_buffer.close()
    return vtt_file
...
async def conver_vtt_to_str(vtt_sub) -> str:
    output = ["WEBVTT"]
    response = ""
    counter = 1
